...
export default function EnrollmentCertificates() {
  ...
  // Фильтрация и удаление регистрации
  ...
  const handleDeleteEnrollment = e => {
    ...
    fetch(`${ENROLLMENTS_API_URL}/${e.id.prisonerId}/${e.id.courseId}`,
  ...
  const handleDeleteCertificate = cert => {
    ...
    fetch(
      `${CERTIFICATES_API_URL}/${cert.id.prisonerId}/${cert.id.courseId}`,
      fetchOptions({ method:'DELETE' })
    )
    .then(r => {
      if (!r.ok) throw new Error();
    ...
    <Paper sx={{ p:3, maxWidth:1200, mx:'auto', my:3, boxShadow:4 }}>
      ...
      {activeTab===1 && (
        <>
                    ...
                    <TableRow key={i} hover>
                      ...
                      <TableCell align="center">
                        <IconButton color="error" onClick={()=>{ 
                          const cert = certificates.find(c=>c.id.prisonerId===e.id.prisonerId&&c.id.courseId===e.id.courseId);
          ...
          {/* Диалог выбор регистрации для сертификата */}
